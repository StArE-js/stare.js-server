...
function calculate(stareDocument, opts) {
  return new Promise((resolve, reject) => {
    ...
    if (_.has(stareDocument, 'body') && stareDocument.body !== null && stareDocument.body !== '') {
      ...
      if (_.has(body, 'edadAFechaLesion')) {
          _.extend(personalData, { edad: _.get(body, 'edadAFechaLesion') })
      }else{
        resolve({
      ...
      if (_.has(body, 'sexoVictima')) {
          _.extend(personalData, { sexo: _.get(body, 'sexoVictima') })
      }else{
        resolve({
          name: 'personal-victim-data',
          index: opts.index,
